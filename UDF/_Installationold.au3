	...
	If($iModeTech = 0 And $idMsgInst = $iIDButtonInstaller And UBound($aSofts) > 0) Then
		...
		Else
			...
			If Not FileExists(@UserProfileDir & '\AppData\Local\Microsoft\WindowsApps\winget.exe') Then
				DirCreate(@LocalAppDataDir & '\bao\tmp')
				FileCopy(@ScriptDir & '\Outils\install-winget.ps1', @LocalAppDataDir & '\bao\tmp\')
				RunWait(@ComSpec & ' /c "powershell.exe -NoProfile -ExecutionPolicy Bypass -File "' & @LocalAppDataDir & '\bao\tmp\install-winget.ps1"', @LocalAppDataDir & '\bao\tmp\', @SW_HIDE)
			EndIf
			If Not FileExists(@UserProfileDir & '\AppData\Local\Microsoft\WindowsApps\winget.exe') Then
