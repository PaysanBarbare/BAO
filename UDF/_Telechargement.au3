	...
	;~ 		EndIf
			...
			Else
				...
				ElseIf $dl = 1 And $logheaders = 1 Then
					...
					$oHTTP.SetRequestHeader("user-agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36")
					...
					$oHTTP.SetRequestHeader("Accept-Language", "fr-FR,fr;q=0.9,en-US;q=0.8,en;q=0.7")
		...
		EndIf
	Else
		_FileWriteLog($hLog, 'Echec de téléchargement, logiciel introuvable')
	EndIf
...
;~  		EndIf
	...
	If($norun = 0) Then
		...
		ElseIf($arg <> "") Then
			...
			If $iPid = 0 And @error <> 0 Then
				_Attention($sProgrun & " a échoué. Si votre antivirus l'a mis en quarantaine, restaurez le maintenant (1)", 1)
				$iPid = ShellExecute($sProgrun, $arg, $sDocp)
			EndIf
		Else
			If(StringInStr (FileGetAttrib ($sProgrun), "D")) Then
				...
				If $iPid = 0 And @error <> 0 Then
					_Attention($sProgrun & " a échoué. Si votre antivirus l'a mis en quarantaine, restaurez le maintenant (2)", 1)
					$iPid = ShellExecute($sProgrun, $arg, $sDocp)
				EndIf
			Else
				$iPid = ShellExecute($sProgrun, "", $sDocp) ; 1.1.2 replace run by shellexecute
				If $iPid = 0 And @error <> 0 Then
					_Attention($sProgrun & " a échoué. Si votre antivirus l'a mis en quarantaine, restaurez le maintenant (3)", 1)
					$iPid = ShellExecute($sProgrun, "", $sDocp)
				EndIf
			EndIf
		EndIf
	Else
